...
#region UTILS
    ...
    class DenoisedFeature : ScriptableRenderPass
    {
        ...
        public void Init(ScriptableRenderer renderer){
            ...
            colorBufferID = renderer.cameraColorTarget;
        }

        //v other variables
        private ScriptableRenderer rendererReference;
        private RenderTargetIdentifier colorBufferID;                    //<- color buffer Identifier for faster access
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            //blits
            cmd.Blit(colorBufferID, rbTEMP.Identifier(), data.effect);                   //< original - 1
            ...
            if(data.additionalBlurIteration){
                ...
                if(data.stage == DebugStage.EFFECT){
                    cmd.Blit(rbTEMP.Identifier(), colorBufferID);
            ...
            cmd.Blit(colorBufferID, rbTEMP3.Identifier(), data.composition);             //original - 3

            //decide the actual frame output for debugging
            switch(data.stage){
                case DebugStage.FULL:
                    cmd.Blit(rbTEMP3.Identifier(), colorBufferID);                       //< 3 - original
                break;
                
                case DebugStage.EFFECT:
                    cmd.Blit(rbTEMP.Identifier(), colorBufferID);                        //< 1 - original
                break;

                case DebugStage.BLUR:
                    cmd.Blit(rbTEMP2.Identifier(), colorBufferID);                       //< 2 - original
                break;
            }
